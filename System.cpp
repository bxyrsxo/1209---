...
void setEncoder ()
{
		...
		IMC_ENC_SetCounter(3,90.0/360*MOTOR_ENCODER_4X[3]*REDUCER50,0);

		IMC_ENC_SetCounter(3,90.0/360*MOTOR_ENCODER_4X[3]*REDUCER100,1);
...
void Output_Voltage( float updateTorque[2][6], int Card_Index )
{
	...
	for(int i=0; i<4; i++) 
	{
		...
		if(outputVoltage[i] > MAX_OUTPUT_VOLTAGE) {outputVoltage[i] = MAX_OUTPUT_VOLTAGE; }  //若輸出電壓太大,則將其設為最大值
		if(outputVoltage[i] < MIN_OUTPUT_VOLTAGE) {outputVoltage[i] = MIN_OUTPUT_VOLTAGE;}  //若輸出電壓太小,則將其設為最小值
